...
- **Porta**: `465` (SSL)
- **Usuário**: `admin@visualdesignmoz.com`
- **Senha**: `Ad.Vd#2425?*`
...
https://visualdesignmoz.com:8090/snappymail/
...
**Soluções:**
- Criar conta `admin@visualdesignmoz.com` no CyberPanel
- OU usar uma conta existente

### 3. Opções de Contas Válidas

#### Opção A: Usar conta existente
Se você tem outra conta como `suporte@visualdesignmoz.com` ou `geral@visualdesignmoz.com`:

1. Verifique o login no webmail
2. Atualize as credenciais no `.env.local`:
```bash
SMTP_MASTER_EMAIL=suporte@visualdesignmoz.com
...
```bash
telnet mail.visualdesignmoz.com 465
```
