...
DOMAINS=$(ssh root@$SERVER "mysql cyberpanel -N -e \"SELECT domain FROM websites_website WHERE domain NOT LIKE '%contaboserver%';\" 2>/dev/null")
...
for DOMAIN in $DOMAINS; do
    ...
    if [ -z "$SPF" ]; then
        ...
        echo "   → Adicione: v=spf1 a mx ip4:109.199.104.22 ~all"
    else
        echo "   ✅ $SPF"
    fi
    ...
    if [ -z "$DKIM_DNS" ]; then
        ...
        echo "   → Ative DKIM no CyberPanel e adicione o registo TXT"
    else
    ...
    PTR=$(dig +short -x 109.199.104.22 2>/dev/null)
    if [ -z "$PTR" ]; then
        echo "   ❌ Nenhum PTR configurado para 109.199.104.22"
    else
        echo "   ✅ $PTR"
    fi
...
echo "  1. ✅ SPF:    v=spf1 a mx ip4:109.199.104.22 ~all"
echo "  2. ✅ DKIM:   Ativar no CyberPanel → Adicionar registo TXT no DNS"
echo "  3. ✅ DMARC:  v=DMARC1; p=none; rua=mailto:postmaster@DOMINIO"
echo "  4. ✅ rDNS:   Contactar Contabo para configurar PTR do IP"
echo ""
echo "🔧 Para ativar DKIM em todos os domínios:"
echo "   ssh root@$SERVER 'for d in \$(mysql cyberpanel -N -e \"SELECT domain FROM websites_website WHERE domain NOT LIKE '%contaboserver%';\"); do cyberpanel issueDKIM --domainName \$d 2>/dev/null; done'"
